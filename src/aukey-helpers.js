 ...
 * @returns {string}
 */
const getScene = ({targetSlotNth, stages = []}) => {
  if (!stages?.length) {
  ...
  const codedStages = stages.map(
    ({
      ...
      if (typeof hue !== 'number' && typeof power !== 'number') {
        throw new Error(
          `I don't know what will happen if there ain't no color nor white!`
        )
      }
  ...
  return [targetSlotNth.toString(16).padStart(2, '0'), ...codedStages].join('')
}
...
const getColor = (
  ...
  /** @type number */ brightneses
) =>
  `${hue.toString(16).padStart(4, '0')}${saturation
    .toString(16)
    .padStart(4, '0')}${brightneses.toString(16).padStart(4, '0')}`
